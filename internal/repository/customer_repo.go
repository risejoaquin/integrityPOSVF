...
func (r *CustomerRepository) List(ctx context.Context, limit, offset int) ([]model.Customer, error) {
	...
	rows, err := r.Pool.Query(ctx, `SELECT id, name, phone, email, metadata, created_at FROM customers ORDER BY created_at DESC LIMIT $1 OFFSET $2`, limit, offset)
	if err != nil {
		return nil, fmt.Errorf("CustomerRepo.List: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		var c model.Customer
		var phone, email *string
		if err := rows.Scan(&c.ID, &c.Name, &phone, &email, &c.Metadata, &c.CreatedAt); err != nil {
			return nil, fmt.Errorf("CustomerRepo.List scan: %w", err)
		}
		if phone != nil {
			c.Phone = *phone
		}
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("CustomerRepo.List rows error: %w", err)
	}
